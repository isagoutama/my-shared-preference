...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.btn_save) {
            ...
            if (isPreferenceEmpty) {
                intent.putExtra(FormActivity.EXTRA_TYPE_FORM, FormActivity.TYPE_ADD);
                intent.putExtra("USER", userModel);
            } else {
                intent.putExtra(FormActivity.EXTRA_TYPE_FORM, FormActivity.TYPE_EDIT);
                intent.putExtra("USER", userModel);
            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE) {
            if (resultCode == FormActivity.RESULT_CODE) {
                userModel = data.getParcelableExtra(FormActivity.EXTRA_RESULT);
                populateView(userModel);
                checkForm(userModel);
            }
        }
